...
public class RoundRobinFiloPolicy implements QueuePolicy {
	...
	private static final Timer FETCH_NEXT_TIMER = ServerMain.getMetricRegistry()
		.timer(MetricRegistry.name("velcom", "queuepolicy", "roundrobinfilo", "fetch"));
	private static final Timer GET_TASKS_TIMER = ServerMain.getMetricRegistry()
		.timer(MetricRegistry.name("velcom", "queuepolicy", "roundrobinfilo", "get"));
	...
	public synchronized Optional<Task> fetchNextTask() {
		try (var ignored = FETCH_NEXT_TIMER.time()) {
	...
	public List<Task> getTasksSorted() {
		try (var ignored = GET_TASKS_TIMER.time()) {
